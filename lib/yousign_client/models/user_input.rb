  ...
  class UserInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `YousignClient::UserInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `YousignClient::UserInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      pattern = Regexp.new(/^\/user_groups\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/)
      if !@group.nil? && @group !~ pattern
        invalid_properties.push("invalid value for \"group\", must conform to the pattern #{pattern}.")
      end

      pattern = Regexp.new(/^\/files\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/)
    ...
    def default_sign_image=(default_sign_image)
      pattern = Regexp.new(/^\/files\/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$/)
      if !default_sign_image.nil? && default_sign_image !~ pattern
        fail ArgumentError, "invalid value for \"default_sign_image\", must conform to the pattern #{pattern}."
      end
