...
module YousignClient
  class MemberOutput
    ...
    attr_accessor :created_at, :updated_at, :status, :file_objects, :operation_level, :operation_custom_modes, :mode_sms_configuration
    ...
    def initialize(attributes = { })
      unless attributes.is_a?(Hash)
        raise ArgumentError, 'The input argument (attributes) must be a hash in `YousignClient::MemberOutput` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({ }) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError, "`#{k}` is not a valid attribute in `YousignClient::MemberOutput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
    ...
    def list_invalid_properties
      invalid_properties = []
      pattern = Regexp.new(%r{^/users/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$})
    ...
    def valid?
      return false if !@user.nil? && @user !~ Regexp.new(%r{^/users/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$})

      type_validator = EnumAttributeValidator.new('String', %w[signer validator])
      return false unless type_validator.valid?(@type)

      status_validator = EnumAttributeValidator.new('String', %w[pending processing done refused])
    ...
    def user=(user)
      pattern = Regexp.new(%r{^/users/[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$})
      if !user.nil? && user !~ pattern
        raise ArgumentError, "invalid value for \"user\", must conform to the pattern #{pattern}."
      end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', %w[pending processing done refused])
