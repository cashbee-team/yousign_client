...
module YousignClient
  class ServerStampInput
    ...
    attr_accessor :config, :file_objects
    ...
    def self.openapi_types
      {
        ...
        file_objects => :'Array<FileObjectInputWithoutFileReference>',
        sign_image   => :String
      }
    end
    ...
    def initialize(attributes = { })
      unless attributes.is_a?(Hash)
        raise ArgumentError, 'The input argument (attributes) must be a hash in `YousignClient::ServerStampInput` initialize method'
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({ }) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError, "`#{k}` is not a valid attribute in `YousignClient::ServerStampInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      if attributes.key?(:sign_image)
        self.sign_image = attributes[:sign_image]
      end
    end
    ...
    def hash
      [file, certificate, config, file_objects, sign_image].hash
    end
